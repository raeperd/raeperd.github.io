...
- [tco-ip-5-layer-network-model](tco-ip-5-layer-network-model.md) 의 Physical, Data Link, Network Layer 의 프로토콜로 하나의 노드에서 다른 노드로의 통신이 가능하다. 
...
- [ethernet](ethernet.md) payload 에 IP Datagram 이 포함되는 것처럼, [IP](ip-address.md) Datagram 의 payload 에 TCP Segment 가 포함된다. 이도 마찬가지로 header와 payload 를 가진다.
...
- [three-way-handshake](three-way-handshake.md)에 사용되는 주요한 필드
